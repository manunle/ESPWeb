...
#include "CustomBtns.h"
#include "Page_BtnConfig.h"
#define DEVICE_TYPE "WebInterface"
...
void ESPBASE::initialize()
{
  ...
  // define parameters storage
    ...
    EEPROM.begin(512); // define an EEPROM space of 1024 Bytes to store data
  #endif

  //**** Network Config load
  CFG_saved = ReadConfig();
  Serial.print("OK, I got config ver ");
...
void ESPBASE::mqttSend(String& topic,String msg)
{
  String preface = ""; 
...
void ESPBASE::httpSetup(){
  ...
  server.on ( "/custombtns.html", send_custombuttons_html  );
  server.on ( "/general.html", send_general_html  );
  server.on ( "/buttoncfg.html", send_btncfg_html );
  server.on ( "/style.css", []() {
  ...
  server.on ( "/admin/custombtns",     send_custombuttons_values_html);
  server.on ( "/admin/toggle",  toggle  );
  server.on ( "/admin/buttonvalues",send_buttoncfg_value_html);
  server.onNotFound ( []() {
